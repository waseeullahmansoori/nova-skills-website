  ...
  if (body) {
    body.innerHTML = `
      ...
      <ol>
        ...
        <li><strong>Leverage Placement Support:</strong> Connect directly with Nova Skills' network of industry hiring connections.</li>
      </ol>
...
function bindCommentForm() {
  ...
  commentForm?.addEventListener('submit', async (e) => {
    ...
    const nameEl = commentForm.querySelector('[name="name"]') || commentForm.querySelector('input[type="text"]');
    const emailEl = commentForm.querySelector('[name="email"]') || commentForm.querySelector('input[type="email"]');
    const msgEl = commentForm.querySelector('textarea');
    ...
    await submitNovaForm({
      ...
      data: {
        ...
        message: msgEl?.value.trim() || 'Blog Article Comment'
      }
    });
  });

  const articleNewsForm = document.getElementById('article-newsletter-form');
  articleNewsForm?.addEventListener('submit', async (e) => {
    e.preventDefault();
    const submitBtn = articleNewsForm.querySelector('button');
    const emailEl = articleNewsForm.querySelector('input[type="email"]');
    ...
    await submitNovaForm({
      ...
      data: {
        email: emailEl?.value.trim() || '',
        message: 'Blog Article Newsletter Subscription'
      }
    });
  });
}
